...
=======
>>>>>>> 57c9ccc1008c290cc0c4067e7aee235339fdf867
...
geneAnnotationData <- getBM(attributes = c("ensembl_gene_id", "entrezgene_id"), 
=======
#get gene annotation data from ensembl database
...
```{r Gene Ontology and Gene Enrichment Analysis, echo = F, error = F}
...
```{r Retrieve Dendrogram Colors for Modules}
...
```{r Calculate Module Eigenvalues for Genes}
# Define numbers of genes and samples
nGenes = ncol(t(gxData));
nSamples = nrow(t(gxData));
...
```{r Plot Module Heatmaps vs Covariates}
sizeGrWindow(20,20)
...
```{r Retrieve Module Correlation and Significance for each Gene (Module Membership)}
# Define variable time containing the time column of datTrait
disease <- sampleInfo_Num;
...
geneTraitSignificance = as.data.frame(WGCNA::cor(t(gxData), disease, use = "p"));
...
```{r Modulewise Plot Gene-Trait Correlation for Disease vs Gene Significance}
