...
=======
```{r dge on sex}
>>>>>>> 2e4f4dfc60826989904547816d49659666d527cc
...
for (Dis in c("DCM", "HCM", "PPCM")) {
  ...
  for (Sx in levels(sampleInfoDis$Sex)) {
    ...
    for (Eth in levels(sampleInfoSx$Ethnicity)) {
      sampleInfoEth <- sampleInfoSx %>% filter(Ethnicity == Eth) %>% mutate(
...
```{r}
=======
## GO and KEGG analysis for each subset in sampleInfoDcmHcm
...
for(set in names(pathwaySubset)) {
  ...
  removeRows <- c(grep(".* - other$", pathwaySubset[[set]]$KEGGResults$Pathway), 
...
# plot multiple pathways (plots saved to disk and returns a throwaway list object)
  ...
  # move new files to respective folders
    ...
    cat("moving files...\n")
    fs::file_copy(path = paste(pid, ".pathview.png", sep = ""), 
    ...
    if (fs::file_exists(paste("./output/pathview_rawPNGs/", pid, ".png", sep = ""))) {
      ...
      fs::file_copy(path = paste(pid, ".png", sep = ""), 
    ...
    if (fs::file_exists(paste("./output/pathview_xmls/", pid, ".xml", sep = ""))) {
      ...
      fs::file_copy(path = paste(pid, ".xml", sep = ""), 
                    new_path = paste("./output/pathview_xmls/", pid, ".xml", sep = ""), 
                    overwrite = F)
    }
      
    fs::file_delete(c(path = paste(pid, ".png", sep = ""),
