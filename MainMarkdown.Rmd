...
foldchanges <- DEAentrez$results$logFC_DonorVsDCM[which(KEGGResults$P.Up < PThreshold | KEGGResults$P.Down < PThreshold)]
setwd("./output")
...
for(set in names(pathwaySubset)) {
  ...
  subDir <- paste(getwd(), "/output/", set, sep="")
  if (file.exists(subDir) == F){dir.create(file.path(subDir))} 
  
  keggresids <- pathwaySubset[[set]]$KEGGResults %>% filter(P.Up < PThreshold | P.Down < PThreshold)
  pathwaySubset[[paste(set)]]$keggresids <- gsub("^path:","" , rownames(keggresids))
  pathwaySubset[[paste(set)]]$foldchanges <- pathwaySubset[[paste(set)]]$DEAResults$results %>% select(1)
  
  sapply(pathwaySubset[[paste(set)]]$keggresids, function(pid) pathview(gene.data = pathwaySubset[[paste(set)]]$foldchangess, pathway.id = pid, species = "hsa"))
}

rm(keggresids, foldchanges, PThreshold)
