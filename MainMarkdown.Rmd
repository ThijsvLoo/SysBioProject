...
# get differentially expressed genes for male and female
...
DEA_MaleDCM <- DEA_Male$results %>% #filter(decideTests_DonorVsDCM != 0) %>% 
  select(grep("DCM", colnames(DEA_Male$results))) %>% mutate(logFC_DonorVsDCM = abs(logFC_DonorVsDCM)) %>% arrange(desc("logFC_DonorVsDCM"))
DEA_FemaleDCM <- DEA_Female$results %>% #filter(decideTests_DonorVsDCM != 0) %>% 
  select(grep("DCM", colnames(DEA_Female$results)))  %>% arrange(desc("logFC_DonorVsDCM"))
...
uniqEntrezGenes <- geneAnnotationData %>% select(c("ensembl_gene_id", "entrezgene_id")) %>% distinct() %>% filter()
  
  column_to_rownames(var = "ensembl_gene_id") %>% right_join(DEA_Male$rankedResultsDCM, by = row.names)


goana(DEA_Male$fittedModel, geneid = uniqueEntrezGenes$entrezgene_id)

getGeneKEGGLinks(species.KEGG = "hsa", convert = FALSE)
getKEGGPathwayNames(species.KEGG = "hsa", remove=TRUE)
kegga()
