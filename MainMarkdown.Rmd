...
DifferentialGeneExpressionCalc <- function(assayData, phenoData, includePPCM = T, saveResultsOnly = T) {
  # Create ExpressionSet object
...
DEA_donorVsDCM <- DifferentialGeneExpressionCalc(gxData, sampleInfo)

```

```{r DEA in male and female DCM patients}
...
DEA_Male <- DifferentialGeneExpressionCalc(gxDataMale, sampleInfoMale, includePPCM = F, saveResultsOnly = F)
...
for (Dis in c("DCM", "HCM", "PPCM")) {
  ...
  for (Sx in levels(sampleInfo$Sex)) {
    sampleInfoSx <- sampleInfoDis %>% filter(Sex == Sx) %>% mutate(Sex = factor(Sex, levels = Sx))
    for (Eth in levels(sampleInfo$Ethnicity)) {
      sampleInfoEth <- sampleInfoSx %>% filter(Ethnicity == Eth) %>% mutate(Ethnicity = factor(Ethnicity, levels = Eth))
...
sampleInfoDcmHcm <- sampleInfoSubsets[-c(grep("PPCM", names(sampleInfoSubsets)))]

rm(sampleInfoSubsets, sampleInfoDonor, sampleInfoDis, sampleInfoEth, sampleInfoSx, Dis, Sx, Eth)
```

## GO and KEGG analysis
=======
```{r filter Duplicate data}
...
##Gene Ontology
>>>>>>> 67b8d2593bdfef3d74e54c23d7f031ba83966f23
...
<<<<<<< HEAD
GOResuls <- goana(DEAentrez$fittedModel)
...
# GO and KEGG analysis for each sampleInfoDcmHcm
pathwaySubset <- list()
...
for (subs in names(sampleInfoDcmHcm)) {
  ...
  DEAentrezSubs <- DifferentialGeneExpressionCalc(gxDataSubs, SInfoSubs, includePPCM = F, saveResultsOnly = F)
  GOsubs <- goana(DEAentrezSubs)
  KEGGsubs <- kegga(DEAentrezSubs)
  subsetData <- list("DEAResults" = DEAentrezSubs, "GOResults" = GOsubs, "KEGGResults" = KEGGsubs)
  pathwaySubset <- c(pathwaySubset, subsetData)
  rm(subsetData)
}
names(pathwaySubset) <- names(sampleInfoSubset)
...
# plot_pathway <- function(pid) pathview(gene.data=foldchanges, pathway.id=pid, species="Hs", new.signature=FALSE)

keggresids <- substr(rownames(kEGGResults), 6, 13)
foldchanges <- DEAentrez$results$logFC_DonorVsDCM
