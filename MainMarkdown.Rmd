...
DifferentialGeneExpressionCalc <- function(assayData, phenoData, includePPCM = T) {
  # Create ExpressionSet object
...
sampleInfoMale <- sampleInfo %>% filter(Sex == "Male") %>% 
  mutate(Disease = factor(Disease)) #, Disease == "DCM" | Disease == "Donor")
gxDataMale <- gxData %>% select(rownames(sampleInfoMale))

sampleInfoFemale <- sampleInfo %>% filter(Sex == "Female") #, Disease == "DCM" | Disease == "Donor")
gxDataFemale <- gxData %>% select(rownames(sampleInfoFemale))

# get differentially expressed genes for male and female
DEA_Male <- DifferentialGeneExpressionCalc(gxDataMale, sampleInfoMale, includePPCM = F)
DEA_Female <- DifferentialGeneExpressionCalc(gxDataFemale, sampleInfoFemale)

# filter for up/down-regulated genes in male and female patients
DEA_MaleDCM <- DEA_Male %>% filter(decideTests_DonorVsDCM != 0) %>% select(grep("DCM", colnames(DEA_MaleDCM))) %>% arrange(desc("logFC_DonorVsDCM"))
DEA_FemaleDCM <- DEA_Female %>% filter(decideTests_DonorVsDCM != 0) %>% select(grep("DCM", colnames(DEA_MaleDCM)))  %>% arrange(desc("logFC_DonorVsDCM"))

write.csv(rownames(DEA_MaleDCM), file = "diffExpGenes_MaleDCM.csv")
write.csv(rownames(DEA_FemaleDCM), file = "diffExpGenes_FemaleDCM.csv")
...
diffGenExprEntrezID <- diffGenExprMerge %>% rownames_to_column(var = "ensembl_gene_id") %>% right_join(geneAnnotationData, by = "ensembl_gene_id")

goana(fit2, universe = NULL, species = "Hs", prior.prob = NULL, covariate=NULL,
      plot=FALSE)
