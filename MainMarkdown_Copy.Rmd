...
sigKEGGResultsSexDCM <- KEGGResultsSexDCM[-removeRows,] %>% filter(P.Up < PThreshold | P.Down < PThreshold)
...
sigKEGGResultsSexDonor <- KEGGResultsSexDonor[-removeRows,] %>% filter(P.Up < PThreshold | P.Down < PThreshold)
...
if (all(gsub("^path:","" , rownames(sigKEGGResultsSexDCM)) == gsub("^path:","" , rownames(sigKEGGResultsSexDonor))) ) {
  ...
  keggresids <- unique(c(gsub("^path:","" , rownames(sigKEGGResultsSexDCM)), gsub("^path:","" , rownames(sigKEGGResultsSexDonor)))) %>% sort()
}
...
foldchanges <- DEAentrezSexDonor$results %>% dplyr::select(logFC_MaleVsFemale) %>% transmute(logFC_MaleVsFemale_Donor = logFC_MaleVsFemale) %>% rownames_to_column(var = "entrez_id") %>% right_join(foldchangesDCM, by = "entrez_id") %>% column_to_rownames(var = "entrez_id")
rm(foldchangesDCM)
...
for (pid in keggresids) {
  pathview(gene.data = foldchanges, pathway.id = pid, species = "hsa", multi.state = T, kegg.dir = getwd(), low = list(gene = "#440154FF"), mid = list(gene = "#21908CFF"), high = list(gene = "#FDE725FF"))#, kegg.dir = dir)
}
```
...
setwd(dir)
for (pid in keggresids) {
  pathview(gene.data = foldchanges, pathway.id = pid, species = "hsa")#, kegg.dir = dir)
}
```
...
sigKEGGresultsSexDis <- KEGGresultsSexDis[-removeRows,] %>% filter(P.Up < PThreshold | P.Down < PThreshold)
keggresids <- gsub("^path:","" , rownames(sigKEGGresultsSexDis))
# keggresids <- substr(rownames(KEGGResultsSex[which(KEGGResultsSex$P.Up < PThreshold | KEGGResultsSex$P.Down < PThreshold),]), 6, 13)

foldchangesSexDis <- DEA_SexDis$results %>% dplyr::select(grep("logFC.*", colnames(DEA_SexDis$results)))
```
```{r}  
dir = "./output/sexcomb"
if (fs::dir_exists(dir)) {
...
col <- viridisLite::viridis(10, option = "E", begin = 0.2)

setwd(dir)
for (pid in keggresids) {
  pathview(gene.data = foldchangesSexDis, pathway.id = pid, species = "hsa", multi.state = T, low = col[1], mid = "grey", high = col[10])#, kegg.dir = dir)
}
```
...
#### GO and KEGG analysis
```{r DCMvsDonor by Sex go and kegg data generation}
