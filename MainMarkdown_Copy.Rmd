...
for (pid in keggresidsDonorDCM) {
  pathview(gene.data = foldchangesDonorDCM, pathway.id = pid, species = "hsa", multi.state = T, kegg.dir = getwd(), low = list(gene = "#440154FF"), mid = list(gene = "#21908CFF"), high = list(gene = "#FDE725FF"))#, kegg.dir = dir)
}
```
...
keggresidsSexDis <- unique(c(gsub("^path:","" , rownames(sigKEGGresultsSexDis)), keggresids))
# keggresids <- substr(rownames(KEGGResultsSex[which(KEGGResultsSex$P.Up < PThreshold | KEGGResultsSex$P.Down < PThreshold),]), 6, 13)

foldchangesDonorDCM <- foldchangesDonorDCM %>% rownames_to_column(var = "entrez_id")
foldchangesSexDis <- DEA_SexDis$results %>% dplyr::select(grep("logFC.*", colnames(DEA_SexDis$results))) %>% rownames_to_column(var = "entrez_id") %>% right_join(foldchangesDonorDCM, by = "entrez_id") %>% column_to_rownames(var = "entrez_id")
```
```{r}  
dir = "./output/sexcomb"
if (fs::dir_exists(dir)) {
